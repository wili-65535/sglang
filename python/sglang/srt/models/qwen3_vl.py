...
class Qwen3VLMoeVisionModel(nn.Module, RotaryPosMixin):
    ...
    def forward(
        ...
        pos_embeds = self.fast_pos_embed_interpolate_from_list(
            grid_thw_list
        )  # wili, original code
        ...
        x = x.unsqueeze(
            1
        )  # wili, [sl,bs=1,h], transpose to [bs,sl,h] in Qwen3_VisionBlock just before attention
...
class Qwen3VLForConditionalGeneration(nn.Module):
    ...
    def __init__(
        ...
        self.visual = Qwen3VLMoeVisionModel(
            ...
            # prefix=add_prefix("model.visual", prefix),  # wili, original code
            prefix=add_prefix("visual", prefix),  # wili, add back this line for QWen3VL
        ...
        if not hasattr(config, "encoder_only") or not config.encoder_only:
            self.model = language_model_cls(
                config=self.config,
                quant_config=quant_config,
                # prefix=add_prefix("model.language_model", prefix),  # wili, original code
                prefix=add_prefix(
                    "model", prefix
                ),  # wili, add back this line for Qwen3VL
            )
